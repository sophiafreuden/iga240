---
title: "IGA 240 - The Final Countdown"
author: "Sophia Freuden"
date: "12/5/2019"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
library(readxl)
library(janitor)
library(lubridate)
library(tidyverse)
```

```{r, warning=FALSE}
nkmissiles <- read_xlsx("nkcleaned2.xlsx")

nkmissiles <- clean_names(nkmissiles)

nkmissiles <- nkmissiles %>%
  mutate(id = f1) %>%
  select(-f1) %>%
  mutate(facility_latitude = as.numeric(facility_latitude)) %>%
  mutate(facility_longitude = as.numeric(facility_longitude)) %>%
  mutate(date = ymd(date))

nkmissiles2 <- nkmissiles %>%
  mutate(date_entered_updated = ymd(date_entered_updated))
```

```{r}
# plot of missile distant over time

# tables of missile launch success rates per missile type

# bar graph of missile types

# GIS render of missile launches animated over time?
```

```{r}
nkmissiles3 <- nkmissiles %>%
  drop_na(distance_travelled)
```

```{r}
nkmissiles4 <- nkmissiles3 %>%
  filter(distance_travelled > 0)
```

```{r nkmissiles distance travelled over time}
ggplot(nkmissiles4, aes(x = date, y = distance_travelled)) + geom_point()
```

```{r}
nkmissiles5 <- nkmissiles2 %>% arrange(desc(distance_travelled)) %>% slice(1:62)
```

```{r}
nkmissiles6 <- nkmissiles2 %>% arrange(desc(distance_travelled)) %>% slice(63:140)
```


